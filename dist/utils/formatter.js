 ...
 */
export function formatProfile(profile, format, section) {
    switch (format) {
        case 'json':
 ...
 */
export function formatProfiles(profiles, format) {
    switch (format) {
        case 'json':
 ...
 */
function formatText(profile, section) {
    const lines = [];
    // Header
    const headerParts = [];
    if (profile.location)
        headerParts.push(`📍 ${profile.location}`);
    if (profile.age)
        headerParts.push(`🎂 ${profile.age}`);
    if (profile.orbitId) {
    ...
    if (profile.worldview) {
        ...
        if (profile.worldview.politics)
            worldviewParts.push(`Politics: ${profile.worldview.politics}`);
        if (profile.worldview.religion)
            worldviewParts.push(`Religion: ${profile.worldview.religion}`);
        if (profile.worldview.causes)
            worldviewParts.push(`Causes: ${profile.worldview.causes}`);
    ...
    if ((!profile.socialHandles || profile.socialHandles.length === 0) && profile.rawDeepSearch) {
        const ds = profile.rawDeepSearch;
 ...
 */
function getSection(profile, section) {
    switch (section.toLowerCase()) {
        ...
        case 'worldview':
            ...
            const parts = [];
            if (profile.worldview.politics)
                parts.push(`Politics: ${profile.worldview.politics}`);
            if (profile.worldview.religion)
                parts.push(`Religion: ${profile.worldview.religion}`);
            if (profile.worldview.causes)
                parts.push(`Causes: ${profile.worldview.causes}`);
            return parts.length > 0 ? '🌍 Worldview: ' + parts.join(' | ') : null;
        case 'accomplishments':
            if (!profile.accomplishments || profile.accomplishments.length === 0)
                return null;
            return ('🏆 Accomplishments:\n' +
                profile.accomplishments.slice(0, 5).map((a) => `  • ${a.name}`).join('\n'));
        case 'controversies':
            if (!profile.controversies || profile.controversies.length === 0)
                return null;
            return ('⚠️ Controversies:\n' +
                profile.controversies.slice(0, 5).map((c) => `  • ${c.name}`).join('\n'));
        case 'passions':
            if (!profile.passions || profile.passions.length === 0)
                return null;
            return '🔥 Passions: ' + profile.passions.map((p) => p.passion).join(', ');
        case 'sources':
            if (!profile.sources || profile.sources.length === 0)
                return null;
            return ('📚 Sources:\n' +
                profile.sources.slice(0, 10).map((s) => `  • ${s.title || s.link} (${s.sourceName || 'unknown'})`).join('\n'));
        default:
            return null;
    }
}
 ...
 */
export function combineProfiles(deepSearchProfile, socialProfile) {
    const combined = {
        name: 'Unknown',
    };
    if (deepSearchProfile) {
        const fullProfile = deepSearchProfile;
        ...
        if (fullProfile.schools && Array.isArray(fullProfile.schools)) {
            combined.education = fullProfile.schools.map((s) => ({
                name: s.name || '',
                years: s.years,
            }));
        }
